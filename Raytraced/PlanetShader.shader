...
*/
Shader "SpaceAssets/PlanetShader"
{
    Properties
    {
        _Albedo ("Sphere Color", color) = (1,0,0,1)
        _AmbientLight ("Ambient lighting", range(0,1)) = 0.4
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                v2f o;
                o.object = v.vertex; //<- start in object coords
            ...
            float3 RayFromObject(float3 object) 
            {
                float3 ray = (object - _VolumeStart) * _VolumeScale;
                return ray;
            }
            
            fixed4 frag (v2f i) : SV_Target
            {
                float4 cameraObject = mul(unity_WorldToObject,float4(_WorldSpaceCameraPos,1.0));
                float3 rayStart = RayFromObject(cameraObject.xyz); // start at the camera
                float3 target = RayFromObject(i.object); // shoot ray towards proxy geometry
                ...
                */
                ...
                float lod=0.0; //<- mipmap level to read (can't use automatic, fails at seam)
                float4 textureColor = tex2Dlod(_SurfaceTexture,float4(texCoords,0,lod));
                float4 oceanMask = tex2Dlod(_SurfaceOcean,float4(texCoords,0,lod));
                ...
                lighting += _AmbientLight; // ambient light
